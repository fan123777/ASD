...
namespace nsAlgorithmsOnGraphs
{
	...
	namespace nsMinimumSpanningTree
	{
		...
		// На основании данных этих таблиц можно заключить, что реализация алгоритма Прима на графах, представленных в виде матрицы смежности лучше всего подходит для разреженных графов, что все другие методы отличаются по производительность от производительности наилучшего возможного случая лишь с точностью до небольшого постоянного множителя(затраты времени на извлечение ребер) на графах средней насыщенности, и что метод Крускала сводит решение этой задачи к сортировке на разреженных графах.
		// Прежде всего, проведенные широкомасштабные исследования позволили разработать более совершенные реализации очереди с приоритетами.
		// Структура данных полного бинарного дерева Фибоначчи(Fibonacci heap), представляющего собой расширение биномиальной очереди, достигает теоретически оптимальной производительности за счет постоянного времени выполнения операции decrease key(уменьшить ключ) и логарифмического времени выполнения операции remove the minimum(удалить минимальное ребро).
		// время выполнения алгоритма Прима становится пропорциональным сумме Е + VlgV.
		// Другой давно известный простой подход, предложенный Д. Джонсоном (D. Jonhson), является наиболее эффективным методом:
		// Свойство 20.12. Пусть задан граф с V вершинами и Е ребрами и пусть d обозначает насыщенность Е / V.
		// Если d < 2, то время выполнения алгоритма Прима пропорционально VlgV.
		// В противном случае мы можем уменьшить время выполнения в худшем случае в lg(E/V) раз, воспользовавшись в качестве очереди с приоритетами /E/V\-арным частично упорядоченным полным деревом.
