...
namespace nsAnalysis
{
	...
	namespace nsAnalysisPrinciples
	{
		...
		// Математически зависимость времени выполнения алгоритма для ввода величиной N от времени выполнения при меньшем количестве вводов легко задается с помощью рекуррентных соотношений.
		...
		// Cn = Cn/2 + 1, где N>=2 и C1 = 1.
		// Решение: CN порядка lg N.
		...
		// Cn = Cn/2 + N, где N>=2 и C1 = 0.
		// Решение: Cn порядка 2N.
		...
		// Cn = 2Cn/2 + N, где N>=2 и C1 = 0.
		// Решение: Cn порядка N lgN. 
		// Формула 2.5 В рекурсивной программе, которая разбивает ввод надвое, а затем производит постоянное количество других операций возникает следующая рекурсия.
		// Cn = 2Cn/2 + 1, где N>=2 и C1 = 1.
		// Решение: Cn порядка 2*N.

		// 2.6 Примеры алгоритмического анализа 
		// Программа 2.1 Последовательный поиск 
		// В данном случае, лучшее, что мы можем гарантировать — будет исследовано не более N чисел.
		...
		// Программа 2.2 Бинарный поиск 
		// Лемма 2.3 Бинарный поиск исследует не более [lgN] + 1 чисел.

		// 2.7 Гарантии, предсказания и ограничения
		// два основных подхода, которые применяют: анализ низкой производительности и анализ средней производительности.
		// Иногда возникает ситуация, когда алгоритм с хорошими характеристиками низкой производительности при работе с практическими данными оказывается медленнее, чем более простые алгоритмы, или же при незначительной разнице в скорости он требует дополнительных усилий для достижения хороших характеристик низкой производительности.
		// Изучение средней производительности алгоритмов более существенно, поскольку оно позволяет делать предположения о времени выполнения программ.
		// Вычислительная сложность — это направление анализа алгоритмов, которое занимается фундаментальными ограничениями, могущими возникнуть при анализе алгоритмов.
		// Когда можно доказать, что время выполнения алгоритма в случае низкой производительности равно O(f(N)), то говорят, что f(N) является верхней границей сложности задачи.
	}
}
