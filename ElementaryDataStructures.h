...
namespace nsDataStructures
{
	...
	namespace nsElementaryDataStructures
	{
		...
		template<typename T>
		T **malloqd(int r, int с);

		// Программа 3.17 Сортировка массива строк
		void program17();
		int compare(const void *i, const void *j);

		// Программа 3.18 Предаавление графа в виде матрицы смежности
		void program18();
		...
		// Этот пример иллюстрирует типовой сценарий обработки строк : символы считываются в большой одномерный массив, указатели сохраняются в отдельных строках, а затем осуществляется управление указателями.
		// Если многомерная матрица является разреженной (sparse) (количество ненулевых элементов относительно невелико), для ее представления вместо многомерного массива можно использовать мультисписок.
		// Граф — это фундаментальный комбинаторный объект, который описывается набором объектов(называемых вершинами) и набором связей между вершинами(называемых ребрами).
		// Один из простых методов представления графа заключается в использовании двумерного массива, называемого матрицей смежности(adjacency matrix).
		// Другой простой метод представления графа предусматривает использование массива связанных списков, называемых списками смежности(adjacency lists).
		// Ориентированный (directed) граф — это граф, у которого соединения между вершинами имеют направление : ребра следуют из одной вершины в другую.
	}
}
