...
namespace nsAlgorithmsOnGraphs
{
	...
	namespace nsDigraphsAndDirectedAcyclicGraph
	{
		...
		// Сначала, однако, мы рассмотрим алгоритм Тарьяна и алгоритм Габова.
		// Во-первых, мы рассматриваем вершины в обратном топологическом порядке, поэтому, когда мы достигнем конца рекурсивной функции для вершины, которую знаем, мы не встретим ни одной вершины из той же сильной компоненты.
		...
		// В то же время, если орграф содержит большие циклы или крупные сильные компоненты(и поэтому содержит базовые DAG небольших размеров), мы можем построить оптимальные либо близкие к оптимальным алгоритмы.
		...
		// Свойство 19.17. Заданы две вершины s и t в орграфе D, пусть sc(s) и sc(t) - соответствующие им вершины в базовом DAG К орграфа D.
		// Тогда t достижима из s в D тогда и только тогда, когда sc(t) достижима из sc(s) в К.
		// Свойство 19.18. Мы можем поддерживать постоянное время запроса АТД абстрактного транзитивного замыкания орграфа при затратах пространства памяти, пропорциональных V + m^2, и времени, пропорциональных Е + m^2 + m*x, на предварительную обработку(на вычисление транзитивного замыкания);
		// здесь m есть число вершин в базовом DAG, a x - число поперечных ребер в его лесе DFS.
		// Свойство 19.19. Мы можем поддерживать постоянное время запроса для транзитивного замыкания любого графа, в базовом DAG которого содержится менее sqrt_3(V) вершин, при затратах на предварительную обработку пространства памяти, пропорционального V, и времени, пропорционального Е + V.

		// 19.10.Перспективы
		// - Доминаторы.
		// Пусть задан DAG все, вершины которого достижимы из одного истока r, вершина v доминирует(dominates) над вершиной t , если каждый путь из г в t содержит s.
		// - Транзитивное сокращение.
		// Пусть задан орграф, требуется найти орграф, который имеет то же транзитивное замыкание и минимальное число ребер среди орграфов этого класса.
		// - Ориентированный эвклидов путь.
		// Существует ли на орграфе путь, соединяющий две заданных вершины, который использует каждое ребро орграфа в точности один раз?
		// - Ориентированный маршрут почтальона.
		// Пусть задан граф, существует ли ориентированный цикл с минимальным числом ребер, который использует каждое ребро графа, по меньшей мере, один раз(при этом он имеет возможность воспользоваться каждым конкретным ребром многократно).
		// - Ориентированный гамильтонов путь.
		// Найти в орграфе простой ориентированный путь максимальной длины.
		// - Односвязный подграф.
		// Если задан орграф и целое число k , определить, существует ли односвязный подграф, имеющий, по меньшей мере, k ребер.
		// - Множество вершин обратной связи.
		// Определить, имеет ли заданный орграф подмножество, состоящее максимум из k вершин, которое содержит, по меньшей мере, одну вершину из каждого направленного цикла в G.
		// - Четные циклы.
		// Определите, содержит ли заданный орграф цикл четной длины.
	}
}
