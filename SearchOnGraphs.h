...
namespace nsAlgorithmsOnGraphs
{
	...
	namespace nsSearchOnGraphs
	{
		...
		// Основное свойство леса поиска DFS, состоит в том, что он представляет пути из каждой вершины в точку, откуда начался поиск связной компоненты, в которой она находится.
		...
		// Свойство 18.11. Поиск в ширину посещает все вершины и ребра графа за время, пропорциональное V^2, в случае представления графа в виде матрицы смежности, и за время, пропорциональное V + Е, в случае представления графа в виде списков смежных вершин.
		// - Кратчайший путь.
		// - Кратчайший путь из единственного источника. 
		// - Кратчайшие пути для всех пар вершин.
		// Понятие кратчайшего пути (shortest path) в графе в общем случае используется для описания соответствующих задач в орграфах и сетях.
		// Дерево имеет небольшую глубину, зато обладает протяженной шириной.
		// Оно может служить иллюстрацией множества особенностей, которыми поиск в ширину на графе отличается от поиска в глубину.Например:
		...
		// Основной принцип достаточно прост: мы снова обращаемся к описанию поиска в ширину из раздела 18.6, но теперь вместо понятия очередь(queue) мы употребляем обобщенный термин бахрома(fringe) для описания множества ребер, которые являются кандидатами для следующего включения в дерево поиска.
		// Начав с петли исходной вершины в бахроме и пустого дерева, до тех пор, пока бахрома не станет пустой, выполняйте следующую операцию:
		// Возьмите очередное ребро из бахромы и перенесите его в дерево.
		// Если вершина, в которую оно ведет, еще не посещалась, пройдите на эту вершину и поместите в бахрому все ребра, которые ведут из этой вершины в вершины, на которых вы еще не были.
		// Возьмите очередное ребро из бахромы и перенесите его в дерево.
		// Посетите вершину, в которую оно ведет, и поместите все ребра, которые выходят из этой вершины, в бахрому, руководствуясь стратегией замены в бахроме, которая требует, чтобы никакие два ребра в бахроме не указывали на одну и ту же вершину.
		// Свойство 18.12.
		// Обобщенный просмотр графа посещает все вершины и ребра графа за время, пропорциональное V^2 в случае представления графа в виде матрицы смежности, и пропорциональное V + Е в случае представления графа в виде списков смежных вершин плюс, в худшем случае, время, затрачиваемое на V операций вставки, V операций удаления и Е операций обновления в очереди размера V.
		// Первая альтернативная стратегия основана на использовании рандомизированной очереди(randomized queue)
		// Другая стратегия, играющая исключительно важную роль в изучении алгоритмов обработки графов, поскольку служит основой целого ряда классических алгоритмов - это использование для бахромы АТД очереди с приоритетами(priority queue)

		// 18.9. Анализ алгоритмов на графах 
		// мы стремимся - в идеальном случае - получить естественные модели ввода данных, обладающие тремя важными свойствами:
		...
		// Аналогично, алгоритмы на графах приводят нас к ситуации, когда во многих приложениях мы вынуждены ходить по тонкому льду в попытках найти нужные решения с учетом всех трех свойств, которые только что были отмечены:
		...
		// Хорошо известно, что по мере возрастания числа ребер, такой фаф сливается, образуя одну гигантскую компоненту.
		// Свойство 18.13. Если Е > (1/2)V*ln(V) + m*V (при положительном значении m), то случайный граф с V вершинами и Е ребрами состоит из одной связной компоненты и изолированных вершин, среднее число которых не превышает e^(-2*m), с вероятностью, приближающейся к 1 при бесконечном возрастании V.
		// Это свойство говорит о том, что мы вполне можем ожидать, что крупные неразреженные случайные графы являются связными.
		// Добавление проверки с целью прекращения выполнения алгоритма, когда известно, что граф представляет собой единую связную компоненту, существенно повышает быстродействие поиска в глубину и алгоритма объединения - поиска(но не поиска в ширину) при обработке насыщенных графов.
		// - Во-первых, из таблицы ясно, что не следует использовать представление графа в виде матрицы смежности в случае разреженных графов больших размеров.
		// - Во-вторых, из таблицы 18.2 легко видеть, что затраты на распределение памяти под узлы списков достигают больших значений, когда списки смежных вершин строятся для крупных разреженных графов.
		// - В третьих, отсутствие числовых значений в столбцах DFS для крупных разреженных графов весьма существенно.
		// - В-четвертых, эта таблица показывает, что метод, в основу которого положен алгоритм объединения - поиска, обладает большим быстродействием, чем поиски в глубину и в ширину, главным образом за счет того, что для него не требуется представление графа целиком.
	}
}
