...
namespace nsDataStructures
{
	...
	namespace nsRecursionAndTrees
	{
		...
		template<typename Item>
		void print(Tree<Item>* tree)
		{
			std::cout << tree->getItem() << std::endl;
		}
		...
		// Определение 5.6 Уровень (level) узла в дереве на единицу выше уровня его родительского узла(корень размещается на уровне 0).
		// Высота(height) дерева — максимальный из уровней узлов дерева. 
		// Длина пути(path length) дерева — сумма уровней всех узлов дерева.
		// Длина внутреннего пути(internal path length) бинарного дерева — сумма уровней всех внутренних узлов дерева.
		// Длина внешнего пути(external path length) бинарного дерева — сумма уровней всех внешних узлов дерева.
		// Лемма 5.7 Длина внешнего пути любого бинарного дерева, имеющего N внутренних узлов, на 27V больше длины внутреннего пути.
		// Лемма 5.8 Высота бинарного дерева с N внутренними узлами не меньше lgN и не больше N — 1.
		// Лемма 5.9 Длина внутреннего пути бинарного дерева с N внутренними узлами не меньше чем Nlg(N/4) и не превышает N(N— 1) / 2.
		...
		// Если вытолкнутая запись является элементом, мы посещаем его; если вытолкнутая запись — дерево, мы выполняем последовательность операций выталкивания, которая зависит от требуемого порядка:
		...
		// Как ни удивительно, обход по уровням можно получить, заменив в программе 5.15 стек очередью, что демонстрирует программа 5.16. 
		// Для реализации прямого обхода используется структура данных типа "последним вошел, первым вышел" (LIFO);
		// для реализации обхода по уровням используется структура данных типа "первым вошел, первым вышел" (FIFO). 

		// 5.7 Рекурсивные алгоритмы бинарных деревьев
		// Сложность реализации и использования таких структур заключается в обеспечении эффективности алгоритмов после выполнения большого числа операций вставки, удаления и других операций.

		// 5.8 Обход графа
		// рекурсивный обход графа, или поиск в глубину (depth-first search).
		// Начиная с любого узла v, мы 
		// - посещаем v;
		// - (рекурсивно) посещаем каждый (непосещенный) узел, связанный с v.
		// Лемма 5.10 Время, требующееся для выполнения поиска в глубину в графе с V вершинами и Е ребрами, пропорционально V + Е, если использовать представление графа в виде списков смежности.
		// Однако для очень больших графов решения union - find все же могут оказаться предпочтительнее, поскольку для представления всего графа требуется объем памяти, пропорциональный Е, в то время как для решений union - find необходим объем памяти, который пропорционален только V.
		// нерекурсивный метод обхода графа с использованием стека.
		// После инициализации стека начальным узлом мы входим в цикл, в котором посещаем узел на верхушке стека, затем обрабатываем его список смежности, помещая в стек каждый узел(если он еще не был посещен) и используя реализацию стека, которая за счет применения стратегии "игнорирования нового элемента" запрещает дублирование.
		// Этот алгоритм обеспечивает посещение всех узлов графа, подобно поиску в глубину, но не является рекурсивным.
		// Например, если вместо стека задействовать очередь, то получится поиск в ширину, который аналогичен обходу дерева по уровням.
